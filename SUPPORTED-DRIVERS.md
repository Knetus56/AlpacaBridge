...
- **ConformU**: 4.5.0 — 0 errors, 0 issues. 4 FAST-target timing warnings (`DeviceState`, `CalibratorState`, `CoverState`, `MaxBrightness`), all clustered in the first ~1.5s after `Connect()` — the same accepted connect-adjacent cold-start window documented for OnStep (async-connect warm-up holding the driver's coarse mutex across several sequential serial round trips), not a driver defect.
